...
async function validateForm(req, res, next) {
  ...
  const {name,phone,place,role,battingStyle,bowlingStyle,tshirt} = req.body;
...
app.post('/pay/upload-photos',  upload.fields([{
  ...
  const user = {name, phone, place, role, battingStyle, bowlingStyle, photo_url, aadhar_url, tshirt, ref_id};
  const status = 0;
