...
app.get('/pay', (req, res) => {
      ...
      db.getConnection(function(err, conn) {
            ...
            const [results, user_prm] = conn.query("SELECT * FROM users WHERE phone = ? AND ref_id = ?",[phone,orderId]);
...
app.post('/pay/callback', (req, res) => {
    console.error("in callback");
      ...
      req.on('data', chunk => {
            body += chunk.toString();
            console.error("in callback body : "+body);
      });
      req.on('end', () => {
console.error("in callback process");
            ...
            if (verifyChecksum) {
                console.error("in callback verified");
                  res.render(__dirname + '/callback.html', { verifySignature: "true", data: postbodyjson });
            }
            else {
                console.error("in callback failed checksum");
